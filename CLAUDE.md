...
- **apps/iptvctl** - Scriptable CLI with public health checks, strict inputs, and idempotent retry IDs over the same live bridge client
...
- **Schema** (`libs/shared/database/src/lib/schema.ts` — canonical; `apps/electron-backend/src/app/database/schema.ts` is a backwards-compat re-export shim):
    ...
    - `mediaQueryCache` (`media_query_cache`) - Fully paged `globalSearch` results, locally cached for up to 72 hours. SQLite triggers clear it whenever content, categories, or playlists change, so imports and metadata changes never leave stale browse results.
